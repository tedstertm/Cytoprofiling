...
infile <- "infile.csv"
outfile1 <- "ExprMatrix.csv"
outfile2 <- "WeightMatrix.csv"
outfile3 <- "LinkList.csv"
expr.matrix <- read.expr.matrix("infile.csv", form="rows.are.samples")
write.csv(expr.matrix, file=outfile1)



weight.matrix <- get.weight.matrix(expr.matrix)
write.csv(weight.matrix, file=outfile2)

link.list <- get.link.list(weight.matrix, report.max=1000)
write.csv(link.list, file=outfile3)

forceNetwork(Links = el, Nodes = nl, Source="from", Target="to",
		   NodeID = "idn",linkWidth = 1, Group = NULL,
